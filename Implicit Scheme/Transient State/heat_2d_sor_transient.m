%Solving 2D Heat Conduction Equation by Successive Relaxation Method
...
if iterative_solver == 3
    ...
    for nt = 1:1400
        ...
        while (error > tol)
            for i = 2:nx-1
                for j = 2:ny-1
                    Temp_orig = Temp(i-1,j) + Temp_old(i+1,j) + Temp(i,j-1) + Temp_old(i,j+1);
                    Temp_seidel = Temp_initial_dt(i,j)/(1+(4*k1)) + ((k1)/(1+(4*k1)))*Temp_orig;
                    Temp(i,j) = (1 - w)*Temp_old(i,j) + w*Temp_seidel;
                end
            end
...
xlabel("X-Axis")
ylabel("Y-Axis")
title(sprintf('No. of Transient State SOR Iterations for IMPLICIT SCH = %d, Total Time = %f seconds',iteration_counter,time));
end        