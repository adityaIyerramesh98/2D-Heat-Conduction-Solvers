%Solving 2D Heat Conduction Equation by Gauss Seidel Method
...
%Computing of 2D SS Heat Conduction Eqn Seidel Solver starts
iterative_solver = 2;
...
if iterative_solver == 2
    ...
    while (error > tol)
        ...
        for i = 2:nx - 1
            ...
            for j = 2:ny - 1
                Temp(i,j) = 0.25*(Temp(i-1,j) + Temp_old(i+1,j) + Temp(i,j-1) + Temp_old(i,j+1));
            end
        end
...
xlabel("X-Axis")
ylabel("Y-Axis")
title(sprintf('No. of SS Iterations for Gauss Seidel Method IMPLICIT SCH = %d',gauss_seidel_iteration));
pause(0.03);